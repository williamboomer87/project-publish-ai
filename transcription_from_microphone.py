...
def reset_system_microphone_file():
    book_dict_before_edited.clear()
    grammar_edited_book.clear()
    tone_edited_book.clear()
    delete_files_in_directory('/tmp/gradio/wave_files/microphone/')
...
def deepgram_transcription(audio_file):
    ...
    with open(audio_file, "rb") as f:
        ...
        json_file_path = f"/tmp/gradio/wave_files/microphone/{file_name}_{random_code}.json"

        with open(json_file_path, "w") as transcript:
            json.dump(result, transcript)

    return "/tmp/gradio/wave_files/microphone/"
...
# Function for transcription form microphone
def read_reocrd_audio_file(audio_file):
    folder_path = "/tmp/gradio/wave_files/microphone"
    ...
    if not os.path.exists(folder_path):
        try:
            ...
            pass
    json_path = deepgram_transcription(audio_file)
    chapter_dict = generate_transcript(json_path)
    whole_book_dict = generate_whole_book(chapter_dict)
...
def check_grammar_spellings_audio():
...
def check_tone_of_the_book_audio(tone):
